...
export default function About() {
    ...
    const [selectedTab, setSelectedTab] = useState(descriptionsData[2].id);
    ...
    console.log(activeEl.getBoundingClientRect())
    if (activeEl) {
        const itemLeft = activeEl.getBoundingClientRect().x;
        const itemWidth = activeEl.getBoundingClientRect().width;
        console.log((itemWidth/2) + itemLeft)
        setIndicatorPos( itemLeft - (itemWidth/2));
    }
  }, [selectedTab]);
  ...
  return (
    ...
    <div  className="about__page bg-dark-1">
        <PageHeader title1="Know about cafe" title2="Regina"
        ...
        <div className="container">
            <div className="about__main text-white">
                <div className='timeline__years d-flex flex-column'  ref={containerRef}>
                    <ul className='year-items list-unstyled'>
                        {
                            descriptionsData.map(item => (
                                <motion.li
                                    key={item.id}
                                    initial={false}
                                    className={`year-item ${selectedTab == item.id ? `is-active` : ''}`}
                                    data-year={item.id}
                        ...
                        <div className='arrow__up'>
                            <motion.img src='./images/about-arrow-up.png' alt='arrow' 
                            animate={{
                                left:indicatorPos
                            }} 
                ...
                {/* <div className="descriptios__years">
                    <ul className="list-unstyled">
                        <li>1927</li>
                    </ul>
                </div>
                <div className="descrption__body">
                    <p className="h3 font-harmond">Lorem Ipsum is simply dummy text 1</p>
                    <p className="description body">By 1927, our small family kitchen had grown into a beloved neighborhood spot. The world was going through uncertain times, yet our doors remained open as a place of comfort and stability. Families gathered here not just to eat, but to feel a sense of normalcy and warmth when life outside felt heavy. The menu expanded slightly, adding dishes that blended tradition with simple creativity, always staying true to the flavors people trusted. Wooden benches filled with laughter, conversations, and the comforting sound of clinking plates. Neighbors began calling us their “second home,” a title we carried with pride. In those years, our restaurant became a sanctuary, a place where food nourished the body, and hospitality nourished the soul.</p>
                </div> */}
            </div>
        </div>
        <DividerLogo />
